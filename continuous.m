...
function phaseout = s3toContinuous(input)
  ...
  for k=1:Nph
    ...
    [dr,dlon,dlat,dv,dgam,dal,dm,da,db,pdyn,hr,nx,ny,nz,thu,cd,cl,rho,p,Tenv,mach,rey1m,trim_fwd,trim_aft,ka_fwd,ka_aft] = dynamics(input.phase(k),input.phase(k).parameter,input.auxdata,k);
